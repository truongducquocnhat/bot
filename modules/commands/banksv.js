...
module.exports.run = async function ({ api, args, event, Users, permssion, Currencies }) {
  ...
  const araxy = await axios.get("https://Ryo-Api-Bank.hteendepzai004.repl.co/")
  if (`${araxy.data.data}` == "false") {
    return api.sendMessage("[ ADMIN ] - ĐÃ ĐẬP SV", threadID, messageID)
  } else {
    if (args[0] == '-r' || args[0] == 'register') {
      const res = await axios.get(`https://Ryo-Api-Bank.hteendepzai004.repl.co/dangky?uid=${senderID}&name=${encodeURI(lmao)}`)
    ...
    if (args[0] == "top") {
      const res = await axios.get(`https://Ryo-Api-Bank.hteendepzai004.repl.co/top`)
      return api.sendMessage(`${res.data.data}`, threadID, messageID)
    }
