...
module.exports.handleReply = async function({ api, args, event, handleReply, Users }) {
  try {
    ...
    switch (handleReply.type) {
      ...
      case "callndh1": {
        if (s.length == 0) {
          api.sendMessage({ body: `⚡Phản hồi từ Người Điều Hành  ${name} đến bạn:\n--------\n${(event.body) || "không có nội dung trả lời"}\n--------\n»💬Phản hồi tin nhắn này để tiếp tục gửi báo cáo về Người Điều Hành `, mentions: [{ tag: name, id: event.senderID }] }, handleReply.id, (e, data) => global.client.handleReply.push({
            name: this.config.name,
            author: event.senderID,
            messageID: data.messageID,
            type: "reply"
          ...
          api.sendMessage({ body: `⚡Phản hồi từ Người Điều Hành  ${name} đến bạn:\n--------\n${(event.body) || "chỉ có tệp không có nội dung trả lời"}\n--------\n»💬Phản hồi tin nhắn này để tiếp tục gửi báo cáo về Người Điều Hành \n--------\nTệp Người Điều Hành  gửi đến bạn!`, attachment: l, mentions: [{ tag: name, id: event.senderID }] }, handleReply.id, (e, data) => global.client.handleReply.push({
            name: this.config.name,
            author: event.senderID,
            messageID: data.messageID,
            type: "reply"
...
module.exports.run = async function({ api, event, Threads, args, Users }) {
  try {
    ...
    api.sendMessage(
      "DONE!\nĐã Đã Gửi Nội Dung Tới " + soad + " Người Điều Hành ...",
      event.threadID,
      () => {
        var idad = global.config.NDH;
        if (s.length == 0) {
