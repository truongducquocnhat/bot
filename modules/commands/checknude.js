...
//if (typeof data.checkNude == "undefined" || data.checkNude == false) return;
  ...
  const ress = await axios.get(`https://api.truccuche27042004.repl.co/imgur?link=${encodeURIComponent(dataa.url)}`)
...
const ress = await axios.get(`https://api.truccuche27042004.repl.co/imgur?link=${encodeURIComponent(event.messageReply.attachments[0].url)}`)
	const link = ress.data.uploaded.image
  var res = await axios.get(`https://manhict.tech/checknude?key=mzk_G8D0BIPFX70FXUYEUL5&link=${link}`);
  var checkIMG = res.data.data
