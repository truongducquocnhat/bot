...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
  ...
  if (args.length == 0) return api.sendMessage(` ===  Bạn có thể dùng  === \n--------\n[𝐌𝐑] => check checkpoint => Lọc thành viên bị bay acc khỏi nhóm\n--------\n[𝐌𝐑] => check del => Lọc thành viên khỏi nhóm theo số tin nhắn\n--------\n[𝐌𝐑] => check onl => Xem thời gian hoạt động bot onl \n--------\n[𝐌𝐑] => check tt => check độ tương tác của bạn\n--------\n[𝐌𝐑] => check box => lọc nhóm dưới 4 thành viên\n--------\n[𝐌𝐑] => check all => check tất cả độ tương tác các thành viên trong box\n--------\n[𝐌𝐑] => check covid => Xem thông tin covid\n--------\n[𝐌𝐑] => check mayman => Xem Tỉ lệ % may mắn của bạn?\n--------\n[𝐌𝐑] => check nude => Check những ảnh gợi cảm\n--------\n    === 「${timeNow}」 ===`, event.threadID, event.messageID);
  var arr = [];
